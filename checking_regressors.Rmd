...
With this, here are some ways to check a regressor, in order of increasing computational and time cost:
...
This method is good, fast, and commonly used. But it is an approximation, so may not give a good estimate of predictive fit.
...
This method assesses predictive fit directly, without having to run many backtests. For example, the method works particularly well when you have many regressors to check. You still just have to fit one backtest (the current forecast with no regressors), then you can use standard techniques for model selection in linear regression, such as recursive feature elimination (make sure you don't eliminate your forecast though)!
...
To forecast this data, we'll fit an [ARIMA model](https://otexts.com/fpp3/expsmooth.html) to this data, which allows regressors to be added. We'll let 

To fit the regression on the regressors we need to check, we'll need to run a backtest on our forecasting algorithm with no regressors. Let's backtest our forecasting model on the last 50 data points, and forecast one-step ahead. The `ARIMA` function will pick the model which has the best AIC value.
